1int와 double은 num타입에 포함된다.
2)var를 사용해 타입을 직접 명시하지 않고 타입 추론을 한다.
3)상수는 fianl키워드를 사용한다.
...
7)static키워드는 클래스 내부에 선언된 함수더라도 정적 메서드가 되며 함수로 볼 수 있다. 최상위 함수처럼 사용할 수 있다.
8)람다식 표현

(number)=>number%2 == 0; //number가 짝수인지 판별
9)선택 매개변수. 함수 정의에서 {}로 감싼 매개변수는 선택적으로 사용할 수 있다. 호출할 때 매개변수명을 값 앞에 써주면 되는데 이런 매개변수를 이름 있는 매개변수라고 한다.
...
10)**객체(object)**란 저장 공간에 할당되어 값을 가지거나 식별자에 의해 참조되는 공간. 이러한 객체를 메모리에 작성하는 것을 인스턴스화한다고 한다. 메모리에 작성된 객체를 인스턴스라고 하고, 인스턴스화하기 위해서는 설계도가 필요한데 설계도 역할을 하는 것이 클래스이다. 클래스 안에는 속성을 표현할 수 있는데 이를 프로퍼티라고 한다.
11)new키워드를 사용해 인스턴스화 시키고, .연산자를 통해 객체의 프로퍼티에 접근할 수 있다.
12)변수명 앞에 **_ 기호 **를 붙이면 외부에서 접근이 불가능 해진다.(private) 이러한 메서드를 접근하기 위해서 게터와 세터메서드를 작성해야 한다.
...
} 13)상속은 extends키워드를 사용하여 부모 클래스의 기존 기능을 재정의할 때 사용한다.
14)추상클래스는 abstract키워드를 사용하고, 추상 클래스는 그대로 인스턴스화할 수 없으며, 다른 클래스에서 임플리먼트하여 기능을 완성하는 상속 재료로 사용되고, 대상 클래스에는 implements키워드를 사용하고, 메서드에는 @override키워드를 사용한다. 여러 추상 클래스를 한 번에 임플리먼트할 수도 있으나 추상 클래스를 구현할 때는 모든 추상 메서드를 꼭 재정의해야 한다. 15)믹스인. with키워드를 사용하면 상속하지 않고 다른 클래스의 기능을 가져오거나 오버라이드할 수 있다. 다형성이라고도 한다.
...
16)다트는 List, Map, Set등의 컬렉션을 제공한다.
...
16-2) Map : 순서가 없고 탐색이 빠른 자료구조 컬렉션이고, 키와 값의 쌍으로 이루어져 있어 키를 이용하여 값을 빠르게 얻을 수 있다. 만약 요청한 키에 해당하는 값이 없다면 값 없음을 의미하는 null을 반환한다.
...
16-3) Set : 집합을 표현하는 자료구조 컬렉션이고, **add(), remove()**메서드로 집합에 추가하거나 삭제를 하고, **contains()**메서드는 찾고자 하는 자료가 집합에 있는지 없는지 불리언 타입으로 반환한다. 리스트와는 다르게 중복을 허용하지 않는다.

var citySet = {'서울','수원','오산','부산'};
citySet.add('안양');
citySet.remove('수원');
print(citySet.contains('서울')); //true
17)dart는 객체 지향 프로그래밍과 함수형 프로그래밍의 특징을 모두 제공한다.
...
something() 함수는 인수로 Function이라는 특수한 클래스의 인스턴스를 받는다. Function키워드는 다트에서 함수를 매개변수로 전달하고자 할 때 사용하는 타입. something() 함수는 내부에서 10이 매개변수로 전달된 f() 함수를 돌려준다. f() 함수는 익명 함수이다. 함수를 매개변수로 전달하기, 수정하기, 변수에 대입하기가 가능한 객체를 일급 객체라고 한다.
...
18) for문은 외부반복 forEach문은 내부반복을 수행한다.
...
19)where 키워드는 조건을 필터링할 때 사용하는 함수이며, map함수는 반복되는 값을 다른 형태로 변환하는 방법을 제공하는 함수이다. .연산자를 찍고 연속적으로 사용하는 메서드 체인을 지원한다.
...
20)toList는 결과를 리스트로 저장하고, 만약 중복되는 값을 제거한 리스트를 얻고 싶으면 toSet를 사용해 세트로 저장을 한다.
...
21)any함수는 리스트에 특정 조건을 충족하는 요소가 있는지 없는지 계산을 할 때 사용한다.
...
23)?. 연산자를 이용하여 null 여부를 간단히 판단할 수가 있다. 객체의 프로퍼티나 메서드에 접근할 때 . 연산자 대신 ?. 연산자를 사용하면 객체가 null일 때 에러를 발생하는 대신 null을 반환한다.
